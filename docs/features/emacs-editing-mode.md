...
**Implemented.** The `emacs` shell option is accepted by `set -o` / `set +o`, the editor lives under [src/interactive/emacs_editing.rs](../../src/interactive/emacs_editing.rs) and [src/interactive/emacs_editing/](../../src/interactive/emacs_editing/), and the `bind` builtin is registered through [src/builtin/bind.rs](../../src/builtin/bind.rs). Every normative requirement of this spec has a test; user-visible behavior is covered by the integration tests under `emacs_mode::*` in [tests/integration/emacs_mode.rs](../../tests/integration/emacs_mode.rs) driven through the shared PTY harness in [tests/integration/interactive_common/](../../tests/integration/interactive_common/), and pure-logic pieces (keymap, kill buffer, undo, inputrc parser, completion-context classifier) are covered by colocated `#[cfg(test)]` unit tests alongside their implementation. The omissions enumerated in Section 15 (kill ring, numeric arguments, keyboard macros, menu completion, ...) are spec-endorsed non-goals, not implementation gaps; their recovery paths are described in Appendix B.
