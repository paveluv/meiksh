...
**Partially implemented.** The `bash_procsub` shell option is accepted by `set -o` / `set +o` ([src/shell/options.rs](../../src/shell/options.rs)) and the lexer in [src/syntax/token.rs](../../src/syntax/token.rs) recognizes the `<(` / `>(` openers when the option is on at parse time, balancing the body and parsing it via the same recursive entry used by `$(...)`. The runtime side lives in [src/shell/proc_substitute.rs](../../src/shell/proc_substitute.rs): each substitution forks a subshell, wires its stdin or stdout through a pipe, and emits a `/dev/fd/N` path word; the consuming command's `execute_simple` exit hook in [src/exec/simple.rs](../../src/exec/simple.rs) closes the fd and reaps the subshell. Lexer / runtime / cleanup behavior is verified by unit tests colocated with each module and by integration tests in [tests/integration/process_substitution.rs](../../tests/integration/process_substitution.rs).

The FIFO-fallback path described in § 6.3 is **not yet** wired up; on a system without `/dev/fd` (none of the supported targets currently) the consuming command would fail to open the path with `ENOENT`. The runtime probe in § 6.1 is therefore a no-op that always emits the `/dev/fd/N` form. Bringing the FIFO path online is tracked in Appendix B as a near-term follow-up.
