...
- Gaps to close:
  - parser-time alias substitution now works for aliases already present in shell state, including blank-terminated alias chaining within simple commands, but aliases defined earlier in the same parsed source are still unavailable until incremental parse/execute is introduced
...
- Gaps to close:
  - alias definitions now feed parser-time expansion for later parses, and blank-terminated aliases can expose the next simple-command word to alias substitution, but same-source alias visibility is not yet POSIX-conformant
