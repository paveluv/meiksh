...
| `REQ-SH-OPERANDS-001` | `docs/posix/issue8/sh-utility.html#tag_20_110_05` | Command-string, `command_name`/`$0`, script-path, and `-s` stdin-with-operands paths | implemented | covered | `src/shell.rs`, `src/sys.rs` | `src/shell.rs`, `src/sys.rs`, `tests/spec/basic.rs`, `tests/differential/portable.rs` | `-c` assigns special parameter 0 from `command_name`, command-file execution assigns `$0` from the operand, and non-interactive stdin now executes incrementally without violating the utility-page no-read-ahead rule while forcing blocking reads back on inherited non-blocking FIFO/terminal stdin. |
...
| `REQ-SH-EXIT-001` | `docs/posix/issue8/sh-utility.html#tag_20_110_14` | `sh` utility exit-status behavior | partial | covered | `src/main.rs`, `src/shell.rs`, `src/exec.rs`, `src/builtin.rs` | `tests/spec/basic.rs`, `tests/differential/portable.rs` | Startup paths now distinguish not-found (`127`) and unrecoverable read-error (`128`) command-file failures, and direct ordinary-builtin errors no longer terminate non-interactive shells, but broader utility-page exit-status/error-classification work remains open. See `GAP-SH-002`. |
| `REQ-SH-ERRORS-001` | `docs/posix/issue8/sh-utility.html#tag_20_110_15` | Consequences of errors for interactive and non-interactive shells | partial | covered | `src/shell.rs`, `src/builtin.rs`, `src/exec.rs`, `src/interactive.rs` | `src/builtin.rs`, `src/exec.rs`, `src/interactive.rs`, `tests/spec/basic.rs`, `tests/differential/portable.rs` | Direct ordinary builtin errors and their redirection failures now stop only the current command, while direct special-builtin redirection failures still terminate non-interactive shells; finer syntax / expansion / assignment / function distinctions remain open. See `GAP-EXEC-001`. |
