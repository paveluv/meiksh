...
| `REQ-SH-OPTIONS-001` | `docs/posix/issue8/sh-utility.html#tag_20_110_04` | Implemented startup options: `-a`, `-c`, `-n`, `-i`, `-f`, `-C`, `-u`, `-v`, `-s`, lone `-`, invalid-option rejection, and named `-o`/`+o` handling for the implemented subset | partial | covered | `src/shell.rs`, `src/expand.rs` | `src/shell.rs`, `src/expand.rs`, `tests/spec/basic.rs` | `meiksh` now supports startup `-a`, `-u`, `-v`, and named `-o`/`+o` forms for `allexport`, `noclobber`, `noglob`, `noexec`, `nounset`, and `verbose`, but the broader Issue 8 option surface is not closed. See `GAP-SH-001`. |
| `REQ-SH-OPERANDS-001` | `docs/posix/issue8/sh-utility.html#tag_20_110_05` | Command-string, `command_name`/`$0`, script-path, and `-s` stdin-with-operands paths | implemented | covered | `src/shell.rs`, `src/sys.rs` | `src/shell.rs`, `src/sys.rs`, `tests/spec/basic.rs` | `-c` assigns special parameter 0 from `command_name`, command-file execution assigns `$0` from the operand, and non-interactive stdin now executes incrementally without violating the utility-page no-read-ahead rule while forcing blocking reads back on inherited non-blocking FIFO/terminal stdin. |
...
| `REQ-SH-EXIT-001` | `docs/posix/issue8/sh-utility.html#tag_20_110_14` | `sh` utility exit-status behavior | partial | covered | `src/main.rs`, `src/shell.rs`, `src/exec.rs`, `src/builtin.rs` | `tests/spec/basic.rs` | Startup paths now distinguish not-found (`127`) and unrecoverable read-error (`128`) command-file failures, and direct ordinary-builtin errors no longer terminate non-interactive shells, but broader utility-page exit-status/error-classification work remains open. See `GAP-SH-002`. |
| `REQ-SH-ERRORS-001` | `docs/posix/issue8/sh-utility.html#tag_20_110_15` | Consequences of errors for interactive and non-interactive shells | partial | covered | `src/shell.rs`, `src/builtin.rs`, `src/exec.rs`, `src/interactive.rs` | `src/builtin.rs`, `src/exec.rs`, `src/interactive.rs`, `tests/spec/basic.rs` | Direct ordinary builtin errors and their redirection failures now stop only the current command, while direct special-builtin redirection failures still terminate non-interactive shells; finer syntax / expansion / assignment / function distinctions remain open. See `GAP-EXEC-001`. |
...
| `REQ-EXPAND-FIELDS-001` | `docs/posix/issue8/shell-command-language.html#tag_19_06_05` | IFS whitespace and non-whitespace field splitting core | partial | covered | `src/expand.rs`, `src/shell.rs` | `src/expand.rs`, `tests/spec/basic.rs` | Core behavior exists, and `$*` now joins using the first character of `IFS`; mixed-quoting and `"$@"`-adjacent corners remain open. See `GAP-EXPAND-001`. |
| `REQ-EXPAND-GLOB-001` | `docs/posix/issue8/shell-command-language.html#tag_19_06_06` | Pathname expansion and `set -f` interaction | implemented | covered | `src/expand.rs` | `src/expand.rs`, `tests/spec/basic.rs` | Dotfile suppression and `set -f` are covered. |
...
| `REQ-BUILTIN-CD-001` | `docs/posix/utilities/cd.html` | `cd` operand handling, `PWD`/`OLDPWD`, `cd -`, and `CDPATH` search | partial | covered | `src/builtin.rs` | `src/builtin.rs`, `tests/spec/basic.rs` | `CDPATH` lookup and required path reporting for non-empty entries now exist. `-L`/`-P`/`-e` and full logical-path fidelity remain open. See `GAP-BUILTIN-001`. |
...
| `REQ-BUILTIN-SET-001` | `docs/posix/utilities/set.html` | Positional parameters and implemented option subset | partial | covered | `src/builtin.rs`, `src/shell.rs`, `src/expand.rs`, `src/exec.rs` | `src/builtin.rs`, `src/expand.rs`, `tests/spec/basic.rs` | `set` now covers `-a`, `-C`, `-f`, `-n`, `-u`, `-v`, named `-o`/`+o` forms for the implemented subset, `set -o` / `set +o` reporting, and `nounset`-driven failures for plain unset parameter expansion, but most of the Issue 8 option surface still remains. See `GAP-BUILTIN-002`. |
...
| `REQ-BUILTIN-UNALIAS-001` | `docs/posix/utilities/unalias.html` | `unalias -a` and missing-name status | partial | covered | `src/builtin.rs` | `src/builtin.rs`, `tests/spec/basic.rs` | Broader diagnostics still need review. |
