...
| `REQ-SH-OPTIONS-001` | `docs/posix/issue8/sh-utility.html#tag_20_110_04` | Implemented startup options: `-a`, `-c`, `-n`, `-i`, `-f`, `-C`, `-u`, `-s`, lone `-`, invalid-option rejection, and named `-o`/`+o` handling for the implemented subset | partial | covered | `src/shell.rs`, `src/expand.rs` | `src/shell.rs`, `src/expand.rs`, `tests/spec/basic.rs` | `meiksh` now supports startup `-a`, `-u`, and named `-o`/`+o` forms for `allexport`, `noclobber`, `noglob`, `noexec`, and `nounset`, but the broader Issue 8 option surface is not closed. See `GAP-SH-001`. |
...
| `REQ-BUILTIN-SET-001` | `docs/posix/utilities/set.html` | Positional parameters and implemented option subset | partial | covered | `src/builtin.rs`, `src/shell.rs`, `src/expand.rs`, `src/exec.rs` | `src/builtin.rs`, `src/expand.rs`, `tests/spec/basic.rs` | `set` now covers `-a`, `-C`, `-f`, `-n`, `-u`, named `-o`/`+o` forms for the implemented subset, `set -o` / `set +o` reporting, and `nounset`-driven failures for plain unset parameter expansion, but most of the Issue 8 option surface still remains. See `GAP-BUILTIN-002`. |
