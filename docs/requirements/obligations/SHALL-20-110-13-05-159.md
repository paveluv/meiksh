...
status: enriched
spec_tested: blocked
testing_blockers: "vi command mode /pattern (backward history search) requires interactive terminal with pty to simulate search and verify results"
...
**Behavior:** The `/pattern<newline>` command shall search backward through history using pattern matching (2.14 notation). `^` at pattern start anchors to line beginning. Not found: alert, no change. Found: set line, cursor at 0. Empty pattern: reuse last. No previous pattern: alert.

**Details:** Same normative content as SHALL-20-110-13-05-104. See `utilities/V3_chap02.html#tag_19_14`.

**Spec reference:** vi Line Editing Command Mode in `utilities/sh.html`, line 643–652.
