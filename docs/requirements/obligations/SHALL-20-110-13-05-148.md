...
status: enriched
spec_tested: blocked
testing_blockers: "vi command mode d motion (delete with motion) requires interactive terminal with pty to verify deletion and save buffer"
refs: []
related_sections: []
tests: []
---
...
**Behavior:** The `d motion` command shall delete characters between cursor and motion target, placing them in save buffer. Count repeats motion. Toward beginning: char under cursor not deleted. `dd` clears line. Excess count: not an error.

**Details:** Same normative content as SHALL-20-110-13-05-082.
