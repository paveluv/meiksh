...
- context-sensitive `!` pipeline negation and improving reserved-word recognition in POSIX grammar-sensitive positions such as `for`
...
The project does **not** yet claim full POSIX conformance. Remaining gaps are tracked in `docs/spec-matrix.md`, with the largest open areas currently around the remaining reserved-word grammar edge cases outside the newly tightened `for` handling, `trap`, broader `set` option coverage, and full job-control/tty semantics.
