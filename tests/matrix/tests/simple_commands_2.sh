...
# a standard utility implemented as a function, variable assignments shall
# affect the current execution environment during the execution of the function.
...
# application is executed with file descriptor 0 not open for reading or with
...
# intrinsic utility (see 1.7 Intrinsic Utilities ), that utility shall be
# invoked.
# REQUIREMENT: SHALL-2-9-1-4-294: Otherwise, the shell shall execute a non-
# built-in utility as described in 2.9.1.6 Non-built-in Utility Execution .
...
# recognized as a declaration utility, then any remaining words after the word
# that expanded to produce the command name, that would be recognized as a
# variable assignment in isolation, shall be expanded as a variable assignment
# (tilde expansion after the first <equals-sign> and after any unquoted <colon>,
# parameter expansion, command substitution, arithmetic expansion, and quote
# removal, but no field splitting or pathname expansion); while remaining words
...
assert_stdout "var=123" \
    "$TARGET_SHELL -c '$test_cmd'"
# ==============================================================================
# REQUIREMENT: SHALL-2-9-1-4-287: If a simple command has a command name and an
# optional list of arguments after word expansion (see 2.9.1.1 Order of
# Processing ), the following actions shall be performed: If the command name
# does not contain any <slash> characters, the first successful step in the
# following sequence shall occur: If the command name matches the name of a
# special built-in utility, that special built-in utility shall be invoked.
# REQUIREMENT: SHALL-2-9-1-4-289: If the implementation has provided a standard
# utility in the form of a function, and that function definition still exists
# (i.e. has not been removed using unset -f or replaced via another function
# definition with the same name), it shall not be recognized at this point.
# REQUIREMENT: SHALL-2-9-1-4-292: Otherwise, the command shall be searched for
# using the PATH environment variable as described in XBD 8.
# REQUIREMENT: SHALL-2-9-1-4-293: Environment Variables : If the search is
# successful: If the system has implemented the utility as a built-in or as a
# shell function, and the built-in or function is associated with the directory
# that was most recently tested during the successful PATH search, that built-in
# or function shall be invoked.
...
# ==============================================================================
# REQUIREMENT: SHALL-2-9-1-6-299: If the execution is being made via the exec
# special built-in utility, the shell shall not create a separate utility
# environment for this execution; the new process image shall replace the
# current shell execution environment.
# REQUIREMENT: SHALL-2-9-1-6-300: If the current shell environment is a subshell
# environment, the new process image shall replace the subshell environment and
# the shell shall continue in the environment from which that subshell
# environment was invoked.
...
# REQUIREMENT: SHALL-2-9-1-6-302: In either case, execution of the utility in
# the specified environment shall be performed as follows: If the command name
# does not contain any <slash> characters, the command name shall be searched
# for using the PATH environment variable as described in XBD 8.
# REQUIREMENT: SHALL-2-9-1-6-303: Environment Variables : If the search is
# successful, the shell shall execute the utility with actions equivalent to
# calling the execl () function as defined in the System Interfaces volume of
# POSIX.1-2024 with the path argument set to the pathname resulting from the
# search, arg0 set to the command name, and the remaining execl () arguments set
# to the command arguments (if any) and the null terminator.
# REQUIREMENT: SHALL-2-9-1-6-307: If the command name contains at least one
# <slash>: If the named utility exists, the shell shall execute the utility with
# actions equivalent to calling the execl () function defined in the System
# Interfaces volume of POSIX.1-2024 with the path and arg0 arguments set to the
# command name, and the remaining execl () arguments set to the command
