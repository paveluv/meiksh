...
# REQUIREMENT: SHALL-2-9-1-1-274:
# Each variable assignment shall be expanded for tilde expansion, parameter
# expansion, command substitution, arithmetic expansion, and quote removal prior
# to assigning the value.

# We test that arithmetic expansion happens inside variable assignments
# preceding
# a simple command.
test_cmd='
...
# REQUIREMENT: SHALL-2-9-1-4-291:
# If the command name matches the name of an intrinsic utility (see 1.7
# Intrinsic Utilities ), that utility shall be invoked.
# REQUIREMENT: SHALL-2-9-1-4-294:
# Otherwise, the shell shall execute a non-built-in utility as described in
# 2.9.1.6 Non-built-in Utility Execution .
# REQUIREMENT: SHALL-2-9-1-4-295:
...
# REQUIREMENT: SHALL-2-9-1-1-270:
# If there is a command name and it is recognized as a declaration utility,
# then any remaining words after the word that expanded to produce the command
# name, that would be recognized as a variable assignment in isolation, shall be
# expanded as a variable assignment (tilde expansion after the first
# <equals-sign> and after any unquoted <colon>, parameter expansion, command
# substitution, arithmetic expansion, and quote removal, but no field splitting
# or pathname expansion); while remaining words that would not be a variable
# assignment in isolation shall be subject to regular expansion (tilde expansion
# for only a leading <tilde>, parameter expansion, command substitution,
# arithmetic expansion, field splitting, pathname expansion, and quote removal).
...
# If a simple command has a command name and an optional list of arguments
# after word expansion (see 2.9.1.1 Order of Processing ), the following actions
# shall be performed: If the command name does not contain any <slash>
...
# REQUIREMENT: SHALL-2-9-1-4-289:
# If the implementation has provided a standard utility in the form of a
# function, and that function definition still exists (i.e. has not been removed
# using unset -f or replaced via another function definition with the same
# name), it shall not be recognized at this point.
# REQUIREMENT: SHALL-2-9-1-4-292:
# Otherwise, the command shall be searched for using the PATH environment
# variable as described in XBD 8.
# REQUIREMENT: SHALL-2-9-1-4-293:
# Environment Variables : If the search is successful: If the system has
# implemented the utility as a built-in or as a shell function, and the built-in
# or function is associated with the directory that was most recently tested
# during the successful PATH search, that built-in or function shall be invoked.
...
# REQUIREMENT: SHALL-2-9-1-4-297:
# If the command name contains at least one <slash>, the shell shall execute a
...
# new process image shall replace the current shell execution environment.
# REQUIREMENT: SHALL-2-9-1-6-300:
...
# be performed as follows: If the command name does not contain any <slash>
# characters, the command name shall be searched for using the PATH environment
# variable as described in XBD 8.
# REQUIREMENT: SHALL-2-9-1-6-303:
# Environment Variables : If the search is successful, the shell shall execute
# the utility with actions equivalent to calling the execl () function as
# defined in the System Interfaces volume of POSIX.1-2024 with the path argument
# set to the pathname resulting from the search, arg0 set to the command name,
# and the remaining execl () arguments set to the command arguments (if any) and
# the null terminator.
# REQUIREMENT: SHALL-2-9-1-6-307:
# If the command name contains at least one <slash>: If the named utility
# exists, the shell shall execute the utility with actions equivalent to calling
# the execl () function defined in the System Interfaces volume of POSIX.1-2024
# with the path and arg0 arguments set to the command name, and the remaining
# execl () arguments set to the command arguments (if any) and the null
# terminator.
# REQUIREMENT: SHALL-2-9-1-6-308:
# If the execl () function fails due to an error equivalent to the [ENOEXEC]
# error, the shell shall execute a command equivalent to having a shell invoked
# with the command name as its first operand, with any remaining arguments
...
# If the named utility does not exist, the command shall fail with an exit
