...
# Utility Syntax Guidelines , with an extension for support of a leading <plus-
# sign> ( '+' ) as noted below.
# REQUIREMENT: SHALL-SH-1021: The following additional options shall be
...
# invokes a command that also uses standard input, the shell shall ensure that
# the standard input file pointer points directly after the command it has read
# when the command begins execution.
# REQUIREMENT: SHALL-STDIN-017: It shall not read ahead in such a manner that
# any characters intended to be read by the invoked command are consumed by the
...
# terminal device and is set to non-blocking reads, then sh shall enable
# blocking reads on standard input.
...
# instead of a leading <hyphen-minus> (meaning the reverse case of the option as
# described in this volume of POSIX.1-2024).
...
# character: The implementation shall attempt to read that file from the current
