...
# delimiter character is one of '<' or '>', the token IO_NUMBER shall result...
# REQUIREMENT: SHALL-2-10-1-397: If the string contains at least three
# characters, begins with a <left-curly-bracket> ('{') and ends ...
...
# But if there's a space, it parses as the command `0` with standard redirection.
test_cmd='echo content > tmp_grammar.txt; 0 <tmp_grammar.txt'
...
# REQUIREMENT: SHALL-2-10-2-417: If the TOKEN begins with '=', then the token
# WORD shall be returned.
# REQUIREMENT: SHALL-2-10-2-418: If all the characters in the TOKEN preceding
# the first such <equals-sign> form a valid name (see XBD...
...
# result.
# REQUIREMENT: SHALL-2-10-2-410: Otherwise, the token WORD shall be returned.
...
# exactly the reserved word esac, the token identifier esac shall result.
# REQUIREMENT: SHALL-2-10-2-408: Otherwise, the token WORD shall be returned.
# REQUIREMENT: SHALL-2-10-2-411: [Third word of for and case] [case only] When
# the TOKEN is exactly the reserved word in, the token identifier in shall result.
