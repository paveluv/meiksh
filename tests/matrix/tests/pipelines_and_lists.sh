...
# command other than that imposed by the underlying system (memory constraints,
# {ARG_MAX}, and so on).
# REQUIREMENT: SHALL-2-9-2-311: For each command but the last, the shell shall
# connect the standard output of the command to the standard input of the next
# command as if by creating a pipe and passing the write end of the pipe as the
# standard output of the command and the read end of the pipe as the standard
# input of the next command.
# REQUIREMENT: SHALL-2-9-2-313: The standard output of command1 shall be
# connected to the standard input of command2 .
# REQUIREMENT: SHALL-2-9-2-315: If the pipeline is not in the background (see
# 2.9.3.1 Asynchronous AND-OR Lists and 2.11 Job Control ), the shell shall wait
# for the last command specified in the pipeline to complete, and may also wait
...
# executed, and so on, until a command has a non-zero exit status or there are
...
# be executed, and so on, until a command has a zero exit status or there are no
# more commands left to execute.
...
# ==============================================================================
# REQUIREMENT: SHALL-Exit Status-316: The exit status of a pipeline shall depend
# on whether or not the pipefail option (see set ) is enabled and whether or not
# the pipeline begins with the ! reserved word, as described in the following
# table.
# REQUIREMENT: SHALL-Exit Status-317: The shell shall use the pipefail setting
# at the time it begins execution of the pipeline, not the setting at the time
# it sets the exit status of the pipeline. (For example, in command1 | set -o
# pipefail the exit status of command1 has no effect on the exit status of the
# pipeline, even if the shell executes set -o pipefail in the current shell
# environment.)
...
# shall form all or part of a foreground job that can be controlled as described
# in 2.11 Job Control .
# REQUIREMENT: SHALL-Exit Status-335: The exit status of a sequential AND-OR
# list shall be the exit status of the last pipeline in the AND-OR list that is
# executed.
...
# REQUIREMENT: SHALL-2-9-3-319: For example, both of the following commands
# write solely bar to standard output: false && echo foo || echo bar true ||
# echo foo && echo bar A ';' separator or a ';' or <newline> terminator shall
# cause the preceding AND-OR list to be executed sequentially; an '&' separator
# or terminator shall cause asynchronous execution of the preceding AND-OR list.
...
# the exit status of the last command that is executed in the list.
...
# ...
# REQUIREMENT: SHALL-Exit Status-343: The exit status of an OR list shall be the
# exit status of the last command that is executed in the list.
...
# ==============================================================================
# REQUIREMENT: SHALL-2-9-264: A command is one of the following: Simple command
# (see 2.9.1 Simple Commands ) Pipeline (see 2.9.2 Pipelines ) List compound-
# list (see 2.9.3 Lists ) Compound command (see 2.9.4 Compound Commands )
# Function definition (see 2.9.5 Function Definition Command ) Unless otherwise
# stated, the exit status of a command shall be that of the last simple command
