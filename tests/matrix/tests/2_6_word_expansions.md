...
#### Test: word in parameter expansion undergoes all expansions

When *word* is evaluated in a parameter expansion like `${parameter:-word}`,
it is subjected to tilde expansion, parameter expansion, command substitution,
arithmetic expansion, and quote removal.

```
begin test "word in parameter expansion undergoes all expansions"
  script
    unset foo
    HOME=/my_home
    # word contains tilde, arithmetic, parameter, command substitution, and quotes
    echo ${foo:-~/$((1+1))-${HOME}-$(echo cmd)-"quoted"}
  expect
    stdout "/my_home/2-/my_home-cmd-quoted"
    stderr ""
    exit_code 0
end test "word in parameter expansion undergoes all expansions"
```
...
#### Test: set -u ignores unset at and star special parameters

When `set -u` is active, attempting to expand an unset parameter causes an error,
except for the `@` and `*` special parameters, which are allowed to be unset
(meaning there are zero positional parameters).

```
begin test "set -u ignores unset at and star special parameters"
  script
    set -u
    set --
    echo "[$@][$*]"
  expect
    ...
    stderr ""
    exit_code 0
end test "set -u ignores unset at and star special parameters"
```
...
#### Test: embedded quoted at joins with adjacent word parts

If `"$@"` is embedded within a word, the first field generated by the expansion
is joined with the beginning part of the original word, and the last field is
joined with the end part of the original word.

```
begin test "embedded quoted at joins with adjacent word parts"
  script
    set -- one two three
    set -- "prefix-$@-suffix"
    echo "$# <$1> <$2> <$3>"
  expect
    stdout "3 <prefix-one> <two> <three-suffix>"
    stderr ""
    exit_code 0
end test "embedded quoted at joins with adjacent word parts"
```
...
#### Test: embedded quoted star joins with adjacent word parts

When `"$*"` is embedded in a larger word, the entire concatenated parameter list
becomes part of that single word.

```
begin test "embedded quoted star joins with adjacent word parts"
  script
    set -- one two three
    IFS="-"
    set -- "prefix-$*-suffix"
    echo "$# <$1>"
  expect
    stdout "1 <prefix-one-two-three-suffix>"
    stderr ""
    exit_code 0
end test "embedded quoted star joins with adjacent word parts"
```
...
#### Test: explicit period in pattern matches leading period

While a leading `<period>` in a filename cannot be matched by `*` or `?`, it
can be explicitly matched by using a `<period>` as the first character of the
pattern.

```
begin test "explicit period in pattern matches leading period"
  script
    d=$(mktemp -d)
    cd "$d" || exit 1
    touch .hidden
    # Use a pattern that matches exactly the file we created to avoid . and .. variation
    echo .hid*
    cd ..
    rm -rf "$d"
  expect
    ...
    stderr ""
    exit_code 0
end test "explicit period in pattern matches leading period"
```
