...
# ==============================================================================
# REQUIREMENT: SHALL-2-7-189: The number n is an optional one or more digit
# decimal number designating the file descriptor number; the application shall
# ensure it is delimited from any preceding text and immediately precedes the
# redirection operator redir-op (with no intervening <blank> characters
# allowed).
# REQUIREMENT: SHALL-2-7-192: The largest file descriptor number supported in
# shell redirections is implementation-defined; however, all implementations
# shall support at least 0 to 9, inclusive, for use by the application.
# REQUIREMENT: SHALL-2-7-193: If the redirection operator is "<<" or "<<-" , the
# word that follows the redirection operator shall be subjected to quote
# removal; it is unspecified whether any of the other expansions occur.
...
# REQUIREMENT: SHALL-2-7-194: For the other redirection operators, the word that
# follows the redirection operator shall be subjected to tilde expansion,
...
# word by a non-interactive shell; an interactive shell may perform it, but if
# the expansion would result in more than one word it is unspecified whether the
# redirection proceeds without pathname expansion being performed or the
# redirection fails.
...
# name results from the expansion of word to be opened for reading on the
# designated file descriptor, or standard input if the file descriptor is not
# specified.
# REQUIREMENT: SHALL-2-7-2-201: The check for existence, file creation, and open
# operations shall be performed atomically as is done by the open () function as
# defined in System Interfaces volume of POSIX.1-2024 when the O_CREAT and
# O_EXCL flags are set, except that if the file exists and is a symbolic link,
# the open operation need not fail with [EEXIST] unless the symbolic link
# resolves to an existing regular file.
# REQUIREMENT: SHALL-2-7-2-203: If the file does not exist, it shall be created
# as an empty file; otherwise, it shall be opened as if the open () function was
# called with the O_TRUNC flag set.
# REQUIREMENT: SHALL-2-7-3-205: The file shall be opened as if the open ()
# function as defined in the System Interfaces volume of POSIX.1-2024 was called
# with the O_APPEND flag set.
...
# word that begins after the next NEWLINE token and continues until there is a
# line containing only the delimiter and a <newline>, with no <blank> characters
# in between.
# REQUIREMENT: SHALL-2-7-4-208: For the purposes of locating this terminating
# line, the end of a command_string operand (see sh ) shall be treated as a
# <newline> character, and the end of the commands string in $( commands ) and `
# commands ` may be treated as a <newline>.
...
# (because the command it is part of is not executed), the here-document shall
# be read without performing any expansions.
# REQUIREMENT: SHALL-2-7-4-215: However, the double-quote character ( '"' )
# shall not be treated specially within a here-document, except when the double-
# quote appears within "$()" , "``" , or "${}" .
# REQUIREMENT: SHALL-2-7-4-219: When a here-document is read from a terminal
# device and the shell is interactive, it shall write the contents of the
# variable PS2 , processed as described in 2.5.3 Shell Variables , to standard
...
# shall be supplied first by the application and shall be read first by the
# shell.
...
# leading <tab> characters shall be stripped from input lines after
# <backslash><newline> line continuation (when it applies) has been performed,
# and from the line containing the trailing delimiter.
# REQUIREMENT: SHALL-2-7-4-217: Stripping of leading <tab> characters shall
# occur as the here-document is read from the shell input (and consequently does
# not affect any <tab> characters that result from expansions).
...
# ==============================================================================
# REQUIREMENT: SHALL-2-7-7-227: The redirection operator: [ n ] <> word shall
# cause the file whose name is the expansion of word to be opened for both
# reading and writing on the file descriptor denoted by n , or standard input if
# n is not specified.
...
# ==============================================================================
# REQUIREMENT: SHALL-2-7-6-224: The redirection operator: [ n ] >& word shall
# duplicate one output file descriptor from another, or shall close one.
# REQUIREMENT: SHALL-2-7-6-225: If word evaluates to one or more digits, the
# file descriptor denoted by n , or standard output if n is not specified, shall
# be made to be a copy of the file descriptor denoted by word ; if the digits in
# word do not represent an already open file descriptor, a redirection error
# shall result (see 2.8.1 Consequences of Shell Errors ); if the file descriptor
# denoted by word represents an open file descriptor that is not open for
# output, a redirection error may result.
...
# ==============================================================================
# REQUIREMENT: SHALL-2-7-5-220: The redirection operator: [ n ] <& word shall
# duplicate one input file descriptor from another, or shall close one.
# REQUIREMENT: SHALL-2-7-5-221: If word evaluates to one or more digits, the
# file descriptor denoted by n , or standard input if n is not specified, shall
# be made to be a copy of the file descriptor denoted by word ; if the digits in
# word do not represent an already open file descriptor, a redirection error
# shall result (see 2.8.1 Consequences of Shell Errors ); if the file descriptor
# denoted by word represents an open file descriptor that is not open for input,
# a redirection error may result.
# REQUIREMENT: SHALL-2-7-5-222: If word evaluates to '-' , file descriptor n ,
# or standard input if n is not specified, shall be closed.
# REQUIREMENT: SHALL-2-7-5-223: Attempts to close a file descriptor that is not
# open shall not constitute an error.
