...
# shall exit
# REQUIREMENT: SHALL-2-8-1-232:
# shall exit 1.
# REQUIREMENT: SHALL-2-8-1-236:
# shall exit 4
# REQUIREMENT: SHALL-2-8-1-244:
# Notes: The shell shall exit only if the special built-in utility is executed
# directly.
# REQUIREMENT: SHALL-2-8-1-246:
# If the exec command fails, a non-interactive shell shall exit from the
# current shell execution environment; an interactive shell may exit from a
# subshell environment but shall not exit if the current shell environment is
# not a subshell environment.
# REQUIREMENT: SHALL-2-8-1-249:
# When this option is on, when any command fails (for any of the reasons listed
# in 2.8.1 Consequences of Shell Errors or by returning an exit status greater
# than zero), the shell immediately shall exit, as if by executing the exit
# special built-in utility with no arguments, with the following exceptions: The
# failure of any individual command in a multi-command pipeline, or of any
# subshell environments in which command substitution was performed during word
# expansion, shall not cause the shell to exit.
# exit.
# REQUIREMENT: SHALL-2-8-1-250:
# If the command_string operand is an empty string, sh shall exit with a zero
# exit status.
...
# ==============================================================================
# REQUIREMENT: SHALL-2-8-1-251:
# The shell shall exit only if the special built-in utility is executed
# directly.
# REQUIREMENT: SHALL-2-8-1-252:
...
# REQUIREMENT: SHALL-2-8-1-231:
# shall not exit.
# REQUIREMENT: SHALL-2-8-1-233:
# Certain errors shall cause the shell to write a diagnostic message to
# standard error and exit as shown in the following table: Error Non-Interactive
# Shell Interactive Shell Shell Diagnostic Message Required Shell language
# syntax error shall exit shall not exit yes Special built-in utility error
# shall exit 1 shall not exit no 2 Other utility (not a special built-in) error
# shall not exit shall not exit no 3 Redirection error with special built-in
# utilities shall exit shall not exit yes Redirection error with compound
# commands shall not exit shall not exit yes Redirection error with function
# execution shall not exit shall not exit yes Redirection error with other
# utilities (not special built-ins) shall not exit shall not exit yes Variable
# assignment error shall exit shall not exit yes Expansion error shall exit
# shall not exit yes Command not found may exit shall not exit yes Unrecoverable
# read error when reading commands shall exit 4 shall exit 4 yes Notes: The
# shell shall exit only if the special built-in utility is executed directly.
# REQUIREMENT: SHALL-2-8-1-234:
# shall not exit
# REQUIREMENT: SHALL-2-8-1-234:
# shall not exit
# REQUIREMENT: SHALL-2-8-1-234:
...
# REQUIREMENT: SHALL-2-8-1-234:
# shall not exit
# REQUIREMENT: SHALL-2-8-1-253:
# The shell is not required to write a diagnostic message, but the utility
# itself shall write a diagnostic message if required to do so.
# REQUIREMENT: SHALL-2-8-1-257:
