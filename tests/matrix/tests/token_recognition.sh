...
# REQUIREMENT: SHALL-2-3-024: These lines shall be parsed using two major modes:
...
# grammar (see 2.10 Shell Grammar ), one or more of the subsequent lines
# immediately following the next NEWLINE token form the body of a here-document
# and shall be parsed according to the rules of 2.7.4 Here-Document .
# REQUIREMENT: SHALL-2-3-026: Any non- NEWLINE tokens (including more io_here
# tokens) that are recognized while searching for the next NEWLINE token shall
# be saved for processing after the here-document has been parsed.
# REQUIREMENT: SHALL-2-3-027: If a saved token is an io_here token, the
# corresponding here-document shall start on the line immediately following the
# line containing the trailing delimiter of the previous here-document.
# REQUIREMENT: SHALL-2-3-028: When it is not processing an io_here , the shell
# shall break its input into tokens by applying the first applicable rule below
# to each character in turn in its input.
# REQUIREMENT: SHALL-2-3-029: At the start of input or after a previous token
# has just been delimited, the first or next token, respectively, shall start
# with the first character that has not already been included in a token and is
# not discarded according to the rules below.
...
# delimited according to one of the rules below.
# REQUIREMENT: SHALL-2-3-031: When both the start and end of a token have been
# delimited, the characters forming the token shall be exactly those in the
# input between the two delimiters, including any quoting characters.
# REQUIREMENT: SHALL-2-3-032: If a rule below indicates that a token is
# delimited, and no characters have been included in the token, that empty token
# shall be discarded.
...
# <backslash>, single-quote, or double-quote or is the first character of an
# unquoted <dollar-sign> single-quote sequence, it shall affect quoting for
# subsequent characters up to the end of the quoted text.
# REQUIREMENT: SHALL-2-3-037: During token recognition no substitutions shall be
# actually performed, and the result token shall contain exactly the characters
# that appear in the input unmodified, including any embedded or enclosing
# quotes or substitution operators, between the start and the end of the quoted
# text.
# REQUIREMENT: SHALL-2-3-038: The token shall not be delimited by the end of the
# quoted field.
# REQUIREMENT: SHALL-2-3-039: If the current character is an unquoted '$' or '`'
# , the shell shall identify the start of any candidates for parameter expansion
# ( 2.6.2 Parameter Expansion ), command substitution ( 2.6.3 Command
# Substitution ), or arithmetic expansion ( 2.6.4 Arithmetic Expansion ) from
...
# expansions or quoting are found nested within the substitution, the shell
...
# tokens are found nested within the substitution, they shall be parsed
# according to the rules of 2.7.4 Here-Document ; if the terminating ')' or '`'
# of the substitution occurs before the NEWLINE token marking the start of the
# here-document, the behavior is unspecified.
...
# REQUIREMENT: SHALL-2-3-044: The token shall not be delimited by the end of the
# substitution.
# REQUIREMENT: SHALL-2-3-050: In situations where the shell parses its input as
# a program , once a complete_command has been recognized by the grammar (see
# 2.10 Shell Grammar ), the complete_command shall be executed before the next
# complete_command is tokenized and parsed.
# REQUIREMENT: SHALL-2-3-1-118: If it is not indicated within a ${...} parameter
...
# commands to be executed are expressed in the language described in 2.
...
# ==============================================================================
# REQUIREMENT: SHALL-2-2-2-005: Enclosing characters in single-quotes ( '' )
# shall preserve the literal value of each character within the single-quotes.
...
# ==============================================================================
# REQUIREMENT: SHALL-2-2-3-007: Enclosing characters in double-quotes ( "" )
# shall preserve the literal value of all characters within the double-quotes,
# with the exception of the characters backquote, <dollar-sign>, and
# <backslash>, as follows: $ The <dollar-sign> shall retain its special meaning
...
# by the double-quotes, but rather shall define the command(s) whose output
...
# REQUIREMENT: SHALL-2-2-3-015: \ Outside of "$(...)" and "${...}" the
# <backslash> shall retain its special meaning as an escape character (see 2.2.1
# Escape Character (Backslash) ) only when immediately followed by one of the
...
# handling of any special characters.
...
# characters from an enclosed "${" to the matching '}' , the double-quotes
# within which the expansion occurs shall preserve the literal value of all
# characters, with the exception of the characters double-quote, backquote,
...
# rules as for characters in double-quotes described in this section except that
# it shall additionally retain its special meaning as an escape character when
# followed by '}' and this shall prevent the escaped '}' from being considered
...
# processed (replaced with the bytes or characters they yield) immediately prior
# to word expansion (see 2.6 Word Expansions ) of the word in which the dollar-
# single-quotes sequence occurs.

# REQUIREMENT: SHALL-2-2-4-019: In cases where a variable number of characters
# can be used to specify an escape sequence ( \x XX and \ ddd ), the escape
# sequence shall be terminated by the first character that is not of the
# expected type or, for \ ddd sequences, when the maximum number of characters
# specified has been found, whichever occurs first.
# REQUIREMENT: SHALL-2-2-4-021: However, implementations shall not replace an
# unsupported character with bytes that do not form valid characters in that
...
# the dollar-single-quote sequence.
