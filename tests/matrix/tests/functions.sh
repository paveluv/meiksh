...
# expansions in 2.6 Word Expansions shall be performed on the text in compound-
# command or io-redirect ; all expansions shall be performed as normal each time
# the function is called.
# REQUIREMENT: SHALL-2-9-5-381: Similarly, the optional io-redirect redirections
# and any variable assignments within compound-command shall be performed during
...
# syntax-error properties described for special built-in utilities in the first
# item in the enumerated list at the beginning of 2.15 Special Built-In
# Utilities .
# REQUIREMENT: SHALL-Exit Status-388: The exit status of a function definition
# shall be zero if the function was declared successfully; otherwise, it shall
...
assert_exit_code_non_zero \
    "$TARGET_SHELL -c '$test_cmd'"
# REQUIREMENT: SHALL-2-9-5-378: The format of a function definition command is
# as follows: fname ( ) compound-command [ io-redirect ... ] The function is
# named fname ; the application shall ensure that it is a name (see XBD 3.216
# Name ) and that it is not the name of a special built-in utility.
# REQUIREMENT: SHALL-2-9-5-383: The compound-command shall be executed whenever
# the function name is specified as the name of a simple command (see 2.9.1.4
...
# ; the special parameter '#' also shall be changed to reflect the number of
# operands.
...
# values they had before the function was executed.
