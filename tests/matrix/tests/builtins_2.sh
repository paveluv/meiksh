...
# interactive shell shall exit from the current shell execution environment; UP
# REQUIREMENT: SHALL-DESCRIPTION-541: If the exec command fails and the shell
# does not exit, any redirections associated with the exec command that were
...
# exec shall not return to the shell; rather, the exit status of the current
# shell execution environment shall be the exit status of utility .
# REQUIREMENT: SHALL-EXIT STATUS-546: If utility is specified and an attempt to
# execute it as a non-built-in utility fails, the exit status shall be as
...
# Consequences of Shell Errors ), the exit status shall be a value in the range
# 1-125.
...
# 2.9.1.6 Non-built-in Utility Execution with utility as the command name and
# the argument operands (if any) as the command arguments.
...
# subshell environment, the shell shall exit from the subshell environment and
...
# shall be determined by the unsigned decimal integer n , if specified.
# REQUIREMENT: SHALL-DESCRIPTION-554: No other actions associated with the
# signal, such as execution of trap actions or creation of a core image, shall
...
# (see 2.5.2 Special Parameters ), except that if the exit command would cause
