...
# The 'shift' Utility
# ==============================================================================
# REQUIREMENT: SHALL-DESCRIPTION-601: -n The shell shall read commands but does
# not execute them; this can be used to check for shell script syntax errors.
# REQUIREMENT: SHALL-DESCRIPTION-602: A user shall explicitly exit to leave the
# interactive shell.
# REQUIREMENT: SHALL-DESCRIPTION-603: This option shall be supported if the
...
# REQUIREMENT: SHALL-DESCRIPTION-605: Enabling vi mode shall disable any other
...
# "$#" down to "$#-n+1" shall be unset, and the parameter '#' is updated to
...
# REQUIREMENT: SHALL-DESCRIPTION-607: -u When the shell tries to expand, in a
# parameter expansion or an arithmetic expansion, an unset parameter other than
# the '@' and '*' special parameters, it shall write a message to standard error
# and the expansion shall fail with the consequences specified in 2.8.1
...
# The 'times' Utility
# ==============================================================================
# REQUIREMENT: SHALL-DESCRIPTION-608: -v The shell shall write its input to
...
# REQUIREMENT: SHALL-DESCRIPTION-626: The four pairs of times shall correspond
# to the members of the <sys/times.h> tms structure (defined in XBD 14.
# REQUIREMENT: SHALL-DESCRIPTION-609: -x The shell shall write to standard error
...
# first operand is an unsigned decimal integer, the shell shall treat all
...
# specified and there are operands, the first operand shall be treated as an
# action and the remaining as conditions.
...
# shell terminates normally (exits), and may occur when the shell terminates
# abnormally as a result of delivery of a signal (other than SIGKILL) whose trap
# action is the default.
# REQUIREMENT: SHALL-DESCRIPTION-637: The environment in which the shell
# executes a trap action on EXIT shall be identical to the environment
# immediately after the last command executed before the trap action on EXIT was
# executed.
...
# REQUIREMENT: SHALL-DESCRIPTION-640: When a subshell is entered, traps that are
# not being ignored shall be set to the default actions, except in the case of a
# command substitution containing only a single trap command, when the traps
# need not be altered.
# REQUIREMENT: SHALL-DESCRIPTION-641: The trap command with no operands shall
# write to standard output a list of commands associated with each of a set of
# conditions; if the -p option is not specified, this set shall contain only the
# conditions that are not in the default state (including signals that were
# ignored on entry to a non-interactive shell); if the -p option is specified,
# the set shall contain all conditions, except that it is unspecified whether
# conditions corresponding to the SIGKILL and SIGSTOP signals are included in
# the set.
# REQUIREMENT: SHALL-DESCRIPTION-643: Otherwise, the list shall contain the
# commands currently associated with each condition.
# REQUIREMENT: SHALL-DESCRIPTION-644: The format shall be: command1 [ ||
# command2 ] ...
# REQUIREMENT: SHALL-DESCRIPTION-645: The shell shall format the output,
# including the proper use of quoting, so that it is suitable for reinput to the
# shell as commands that achieve the same trapping results for the set of
# conditions included in the output, except for signals that were ignored on
# entry to the shell as described above.
# REQUIREMENT: SHALL-DESCRIPTION-646: If this set includes conditions
# corresponding to the SIGKILL and SIGSTOP signals, the shell shall accept them
# when the output is reinput to the shell (where accepting them means they do
# not cause a non-zero exit status, a diagnostic message, or undefined
# behavior).
# REQUIREMENT: SHALL-DESCRIPTION-647: The trap special built-in shall conform to
...
# the proper use of quoting, so that it is suitable for reinput to the shell as
# commands that achieve the same trapping results for the specified set of
# conditions.
# REQUIREMENT: SHALL-DESCRIPTION-610: The default for all these options shall be
# off (unset) unless stated otherwise in the description of the option or unless
# the shell was invoked with them on; see sh .
# REQUIREMENT: SHALL-DESCRIPTION-614: The command set -- without argument shall
...
# the implementation does not perform the optional check described above for a
# command substitution containing only a single trap command, and no trap
# commands with operands have been executed since entry to the subshell, the
# list shall contain the commands that were associated with each condition
