...
# REQUIREMENT: SHALL-UMASK-1325:
# In a symbolic_mode value, the permissions op characters '+' and '-' shall be
# interpreted relative to the current file mode creation mask; '+' shall cause
# the bits for the indicated permissions to be cleared in the mask; '-' shall
# cause the bits for the indicated permissions to be set in the mask.
# REQUIREMENT: SHALL-UMASK-1326:
# The file mode creation mask shall be set to the resulting numeric value.
# REQUIREMENT: SHALL-UMASK-1329:
...
# REQUIREMENT: SHALL-UMASK-1331:
# If -S is specified, the message shall be in the following format:
# "u=%s,g=%s,o=%s\n", < owner permissions >, < group permissions >, < other
# permissions > where the three values shall be combinations of letters from the
