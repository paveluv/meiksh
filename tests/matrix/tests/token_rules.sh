...
# used as the first character of a new operator, the current token (if any)
# shall be delimited.
...
# operator and the current character is not quoted and can be used with the
# previous characters to form an operator, it shall be used as part of that
# (operator) token.
# REQUIREMENT: SHALL-2-3-035: If the previous character was used as part of an
# operator and the current character cannot be used with the previous characters
# to form an operator, the operator containing the previous character shall be
# delimited.
...
# any token containing the previous character is delimited and the current
...
# subsequent characters up to, but excluding, the next <newline> shall be
...
# input instead of the TOKEN , with token recognition (see 2.3 Token Recognition
# ) resuming at the start of the alias value.
# REQUIREMENT: SHALL-2-3-1-053: An implementation may defer the effect of a
# change to an alias but the change shall take effect no later than the
# completion of the currently executing complete_command (see 2.10 Shell Grammar
# ).
...
# POSIX.1-2024, alias definitions shall not be inherited by separate invocations
# of the shell or by the utility execution environments invoked by the shell;
